...
$name = ucfirst(strtolower($_POST['item_name']));
/***************************************************************************************************************************/
try
{
	// On se connecte à MySQL //le faire qu'une fois au début
...
$req = $bdd->prepare('INSERT INTO inventaire_ndl(item_name, item_etat,item_photo,item_qr,item_unite) VALUES(:name, :etat,:photo,:qr,:unite)');

$req->execute(array(
	'name' => $name,
	'etat' => $_POST['item_etat'],
	'photo' => $name,
	'qr' => $name,
	'unite' => $_POST['item_unite']
));
/*****************************************PARTIE IMAGE******************************************/
$error_messages = [];
...
if (!empty($_GET['submit'])) {
  ...
  if (!isset($_POST['submit'])) {
    ...
    if ($valid) {
      $target_dir = "image/item_photo/IMG_" . $name . "_";
